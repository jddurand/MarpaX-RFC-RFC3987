...
# 99.9% a copy of URI-1.69/abs.t but blame ME and NOT URI in case of a bug
...
while (<DATA>) {
   ...
   my $bad;
   my $u = MarpaX::RFC::RFC3987->new($uref, $base);
   ...
   $u = MarpaX::RFC::RFC3987->new($uref);
   my $b = MarpaX::RFC::RFC3987->new($base);
   if ($u->abs($b,1) ne $expect && $uref !~ /^http:/) {
       $bad++;
       my $abs = $u->abs($b, 1)->as_string;
       print qq(MarpaX::RFC::RFC3987->new("$uref")->abs(MarpaX::RFC::RFC3987->new("$base"), 1) ==> "$abs" (not "$expect")\n);
   }
